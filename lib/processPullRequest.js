...
const processPullRequest = async ({ owner, repo, number, commit_id }) => {
  try {
    ...
    const approve = !eslintViolations && !stylelintViolations;
    const body = approve ? `No violations found.` : `${[eslintViolations && `ESLint`, stylelintViolations && `stylelint`].filter(s => s).join(` and `)} violations found.`;
    const review = {
      owner,
      repo,
      number,
